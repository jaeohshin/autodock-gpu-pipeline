...
Input Files:
    - docking_results_clean_20251031_143707.csv
      Docking scores for all compounds against all receptor structures
      Columns: kinase, structure_id, compound_id, compound_type, docking_score, 
               mean_score, std_score, n_poses
    
    - conformational_state_counts_structure_mapping.csv
      Conformational state annotation for each receptor structure
      Columns: kinase, structure_id, filename, conformational_state

Output Files:
    - individual_structure_performance.csv
      Performance metrics (EF1%, EF5%, EF10%, AUC) for each receptor structure
      Columns: kinase, structure_id, conformational_state, ef1, ef5, ef10, auc,
               n_actives, n_decoys, n_total
    
    - structure_performance_analysis.png
      Visualization of performance distributions by conformational state
"""
"""
Structure-Performance Correlation Analysis
Merges docking results with conformational state data and analyzes:
...
def create_visualizations(performance_df):
    ...
    plt.tight_layout()
    plt.savefig('../data/structure_performance_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def main():
    ...
    mapping_file = '../data/conf_state_structure_mapping.csv'
    ...
    performance_df.to_csv('../data/individual_structure_performance.csv', index=False)
