...
# Read BioEmu data
bioemu_df = pd.read_csv('../data/individual_structure_performance.csv')

# Read your single crystal structure data
your_data = pd.read_csv('../data/all_kinases_ef_summary.csv')
...
for i, (bioemu_col, crystal_col, label_pct) in enumerate(ef_columns):
    ...
    # Save as PDF
    filename_pdf = f'../data/cry_vs_bioemu_{crystal_col.lower().replace("%", "percent")}.pdf'
