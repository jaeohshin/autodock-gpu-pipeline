...
GRID_DIR = "../virtual_screening/grids/crystal"
...
def run_vs_for_kinase(kinase):
    kinase_lig_dir = os.path.join(INPUT_DIR, "ligands_sdf2meeko", kinase)
    receptor_pdb_dir = os.path.join(INPUT_DIR, "receptor_crystal", kinase)
    receptor_pdbqt_dir = os.path.join(PREPROCESS_DIR, "receptor_crystal_pdbqt", kinase)
    grid_center_dir = os.path.join(PREPROCESS_DIR, "grid_crystal_centers", kinase)
    fld_dir = os.path.join(GRID_DIR, kinase)
    out_dir = os.path.join(OUT_DIR, "crystal", kinase)
    ...
    receptor_files = [f for f in os.listdir(receptor_pdb_dir) if f.endswith(".pdb")]
    for receptor_file in receptor_files:
        # Since only one receptor.pdb, set fixed index
